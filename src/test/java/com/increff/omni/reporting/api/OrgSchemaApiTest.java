...
public class OrgSchemaApiTest extends AbstractTest {
    ...
    public void testMapOrgSchema() {
        OrgSchemaPojo pojo = getOrgSchemaPojo(1, 100001);
        api.map(pojo);
        List<OrgSchemaPojo> pojoList = api.selectAll();
        assertEquals(1, pojoList.size());
        assertEquals(1, pojoList.get(0).getOrgId().intValue());
        assertEquals(100001, pojoList.get(0).getSchemaId().intValue());
    }
    ...
    public void testMapExistingOrgSchema() {
        ...
        assertEquals(1, pojoList.get(0).getOrgId().intValue());
        assertEquals(100002, pojoList.get(0).getSchemaId().intValue());
        assertEquals(2, pojoList.get(1).getOrgId().intValue());
        assertEquals(100001, pojoList.get(1).getSchemaId().intValue());
    }
    ...
    public void testGetCheckByOrgId() throws ApiException {
        OrgSchemaPojo pojo = getOrgSchemaPojo(1, 100001);
        api.map(pojo);
        OrgSchemaPojo p = api.getCheckByOrgId(1);
        assertEquals(1, p.getOrgId().intValue());
        assertEquals(100001, p.getSchemaId().intValue());
    }
